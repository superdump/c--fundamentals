...
class WeightedGraph {
 public:
    ...
    friend std::ostream& operator<<(std::ostream& out,
        const WeightedGraph<U>& g);
    WeightedNode<T>* createNode(const T& v);
    void generate(int n, double density, int maxWeight = 100);
    bool isConnected();
    int dijkstrasShortestPath(const T& a, const T& b);
    std::shared_ptr<TreeNode> MSTPrim();
 private:
    std::vector<std::unique_ptr<WeightedNode<T> > > nodes;
};

template <class T>
WeightedNode<T>* WeightedGraph<T>::createNode(const T& v) {
    nodes.emplace_back(
        std::unique_ptr<WeightedNode<T> >(new WeightedNode<T>(v)));
...
template <class T>
void WeightedGraph<T>::generate(int n, double density, int maxWeight) {
    if (nodes.size() > 0) {
        nodes.clear();
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        nodes.emplace_back(
            std::unique_ptr<WeightedNode<T> >(new WeightedNode<T>(i)));
    }
    ...
    for (const auto& nodeA : nodes) {
        ...
        for (const auto& nodeB : nodes) {
            ...
            if (nodeA == nodeB || nodeA->isConnected(nodeB.get()) ||
                    nodeB->isConnected(nodeA.get())) {
                continue;
            }
...
std::ostream& operator<<(std::ostream& out,
        const std::unordered_set<WeightedNode<T>*>& nodes) {
...
template <class T>
bool WeightedGraph<T>::isConnected() {
    std::unordered_set<WeightedNode<T>*> open;
    std::unordered_set<WeightedNode<T>*> closed = { nodes[0].get() };
...
using WeightedEdgeTuple =
    std::tuple<std::shared_ptr<TreeNode>, WeightedNode<T>*, int>;

template <class T>
class WeightedEdgeComparator {
 public:
    bool operator()(
            const WeightedEdgeTuple<T>& lhs,
            const WeightedEdgeTuple<T>& rhs)
            const {
...
template <class T>
int WeightedGraph<T>::dijkstrasShortestPath(const T& a, const T& b) {
    ...
    std::priority_queue<
        WeightedEdgeTuple<T>,
        std::vector<WeightedEdgeTuple<T> >,
        WeightedEdgeComparator<T> > open;
    std::unordered_set<WeightedNode<T>*> closed = {nodes[a].get()};
...
template <class T>
std::shared_ptr<TreeNode> WeightedGraph<T>::MSTPrim() {
    std::priority_queue<
        WeightedEdgeTuple<T>,
        std::vector<WeightedEdgeTuple<T> >,
        WeightedEdgeComparator<T>> open;
...
template <class U>
std::ostream& operator<<(std::ostream& out, const WeightedGraph<U>& g) {
